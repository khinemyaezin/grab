package com.inventory.adapter.persistence.specification.jpa;
...
import com.inventory.adapter.persistence.entity.meta.InventoryItemEntity_;
import com.inventory.adapter.persistence.entity.meta.LocationEntity_;
...
public class InventorySearchSpecification {
    ...
    public Optional<InventoryItemView> findById(String inventoryItemId) {
        ...
        dataQuery.where(
                cb.equal(root.get(InventoryItemEntity_.UUID), inventoryItemId),
                cb.equal(root.get(InventoryItemEntity_.LOCATION_ID), location.get(LocationEntity_.UUID))
        );
        List<InventoryItemView> results = entityManager.createQuery(dataQuery).setMaxResults(1).getResultList();
