package com.identity.domain.policy.impl;

import com.identity.domain.aggregate.Platform;
import com.identity.domain.exception.IdentityDomainError;
import com.identity.domain.exception.IdentityDomainValidationException;
import com.identity.domain.policy.AccessPlacementPolicy;
import com.identity.domain.service.MerchantAccessProfile;
import com.identity.domain.valueobject.AccessScope;

import java.util.Objects;

public final class MerchantOwnerAccessPlacementPolicy implements AccessPlacementPolicy {
    @Override
    public String placementRoleCode() {
        return MerchantAccessProfile.OWNER_ROLE_CODE;
    }

    @Override
    public AccessPlacementPlan plan(
            Platform platform,
            AccessScope targetScope
    ) {
        Objects.requireNonNull(platform, "platform is required");
        Objects.requireNonNull(targetScope, "targetScope is required");

        requireSellerPlatform(platform);
        requireMerchantScope(targetScope);

        String applicantRole = platform.requireSupportedRole(
                MerchantAccessProfile.APPLICANT_ROLE_CODE
        );
        String ownerRole = platform.requireSupportedRole(
                MerchantAccessProfile.OWNER_ROLE_CODE
        );

        return new AccessPlacementPlan(
                applicantRole,
                AccessScope.global(),
                ownerRole,
                targetScope
        );
    }

    private void requireSellerPlatform(Platform platform) {
        if (!MerchantAccessProfile.SELLER_PLATFORM_CODE
                .equals(platform.getCode())) {
            throw new IdentityDomainValidationException(
                    new IdentityDomainError.InvalidAccessCode(
                            "platformCode",
                            platform.getCode()
                    ),
                    "Merchant owner placement requires the seller platform"
            );
        }
    }

    private void requireMerchantScope(AccessScope targetScope) {
        if (targetScope.isGlobal()
                || !MerchantAccessProfile.MERCHANT_SCOPE_KEY
                .equals(targetScope.key().value())) {
            throw new IdentityDomainValidationException(
                    new IdentityDomainError.InvalidAccessScope(
                            targetScope.key().value(),
                            targetScope.scopeId()
                    ),
                    "Merchant owner placement requires a merchant account scope"
            );
        }
    }
}
