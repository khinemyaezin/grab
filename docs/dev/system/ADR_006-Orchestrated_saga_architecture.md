...
- **Phase 3:** Migrate remaining hand-written orchestrators onto the engine; keep CQRS for single-BC writes. *(update-sellable-product and update-product-variant: landed on `WorkflowProcess` definitions + `EventDrivenWorkflowEngine`.)*
