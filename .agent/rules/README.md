...
26. Outbound API exposure via shared interface ports (instead of gRPC): interface lives in `{domain}/query/` with DTOs declared inside the interface, adapter in `{domain}/internal/api/adapter/{port}Adapter` (or `{domain}/internal/api/query/{port}Adapter`) delegating only to `*UseCase` with transaction starting on the adapter method, and mapper in `{domain}/internal/api/query/mapper/{QueryPortName}Mapper`.
