...
- Mess with each other (no cross-calling between command and query services):
  ...
  - `CommandService` MUST NOT perform queries. If business logic needs aggregate data, the command handler must load the aggregate via domain repository, or required inputs must be provided in the request payload.
...
- Inject or call repositories (domain or infrastructure). Only handlers touch repositories.
