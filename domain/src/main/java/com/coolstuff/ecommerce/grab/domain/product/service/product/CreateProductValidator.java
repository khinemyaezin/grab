...
class CreateProductValidator implements Validator<PersistableProduct> {
    ...
    public void validate(PersistableProduct input) {
        ...
        Assert.notNull(input.getCategory(),"Category is required");
        Assert.notNull(input.getCategory().getUuid(),"Category is required");
        Assert.hasLength(input.getCategory().getUuid(),"Category is required");
    }
}
