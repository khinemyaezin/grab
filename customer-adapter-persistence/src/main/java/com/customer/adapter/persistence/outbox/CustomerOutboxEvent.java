...
@Setter
@Entity
@Table(name = "customer_outbox_events", indexes = {
        @Index(name = "idx_customer_outbox_status_available", columnList = "status, available_at"),
        @Index(name = "idx_customer_outbox_claimed_at", columnList = "claimed_at"),
        @Index(name = "idx_customer_outbox_aggregate", columnList = "aggregate_type, aggregate_id")
})
public class CustomerOutboxEvent implements OutboxEntry<Long> {
    @Id
    ...
    public static CustomerOutboxEvent pending(
            String aggregateType,
            String aggregateId,
            SerializedEvent serialized,
            LocalDateTime now
    ) {
        CustomerOutboxEvent event = new CustomerOutboxEvent();
